...
export const Route = createFileRoute("/blog")({
  head: () => ({
    meta: [
      { title: "Insights | SoftSkySolution" },
...
const posts: Post[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "Finally, anti-fraud. We do not pretend to have machine learning answers; instead, we use simple, transparent signals, like phone verification, device fingerprints, and transaction velocity limits, which catch 90% of bad actors with 10% of the engineering cost.",
    ],
    takeaways: [
      ...
      <section className="relative py-16 bg-gradient-hero overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 md:px-6 text-center max-w-3xl">
          ...
          <h1 className="text-5xl md:text-6xl font-display font-bold mb-4">
            SoftSky <span className="text-gradient-primary">Insights</span>
          </h1>
        ...
        <DialogContent className="max-w-3xl w-[95%] p-0 overflow-hidden bg-card border-border/60 shadow-2xl rounded-xl max-h-[90vh] flex flex-col z-[100]">
          {openPost && (
            <>
              ...
              <div className="p-6 md:p-10 overflow-y-auto space-y-5 text-left">
                <DialogHeader className="space-y-3">
                  <DialogTitle className="text-2xl md:text-3xl font-display font-bold leading-tight text-left">
                    {openPost.title}
                  </DialogTitle>
                  <div className="flex items-center gap-4 text-xs text-muted-foreground justify-start">
                    <span className="flex items-center gap-1.5">
                      <Calendar className="h-3.5 w-3.5" /> {openPost.date}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Clock className="h-3.5 w-3.5" /> {openPost.read}
                    </span>
                  </div>
                </DialogHeader>
